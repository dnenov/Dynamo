...
namespace Dynamo.Wpf.Utilities
{
    internal static class ImageUtilities
    {
        ...
        /// <summary>
        ///     Save a bitmap image to jpg or png, depending on the file path
        /// </summary>
        /// <param name="image">Bitmap image to be saved</param>
        /// <param name="filePath">Full image file path including name and extension</param>
        public static void SaveBitmap(Bitmap image, string filePath)
        {
            if (image == null) return;

            if(Path.GetExtension(filePath) == ".jpg")
            {
                image.Save(Path.Combine(filePath), ImageFormat.Jpeg);
                return;
            }

            if (Path.GetExtension(filePath) == ".png")
            {
                image.Save(Path.Combine(filePath), ImageFormat.Png);
                return;
            }
